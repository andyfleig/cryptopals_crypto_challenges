...
func TestChallenge9(t *testing.T) {
	...
	if bytes.Compare(res, []byte("YELLOW SUBMARINE\x04\x04\x04\x04")) != 0 {
		t.Error("c9: wrong result", res)
	}

	res = addPKCSPadding([]byte("YELLOW SUBMARINE"), 16)
	if bytes.Compare(res, []byte("YELLOW SUBMARINE")) != 0 {
		t.Error("c9: wrong result", res)
	}
...
func TestChallenge15(t *testing.T) {
	if string(removePKCSPadding([]byte("ICE ICE BABY\x04\x04\x04\x04"))) != "ICE ICE BABY" {
		t.Error("c15: wrong result")
	}
	if removePKCSPadding([]byte("ICE ICE BABY\x05\x05\x05\x05")) != nil {
		t.Error("c15: wrong result")
	}
	if removePKCSPadding([]byte("ICE ICE BABY\x01\x02\x03\x04")) != nil {
		t.Error("c15: wrong result")
	}
}
